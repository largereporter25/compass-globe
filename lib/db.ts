// Neon Postgres over HTTP. The app degrades gracefully with no DATABASE_URL:
// analysis still runs, nothing is persisted, and the UI says so plainly.

import { neon } from "@neondatabase/serverless";

export const dbEnabled = Boolean(process.env.DATABASE_URL);

export const sql = dbEnabled ? neon(process.env.DATABASE_URL!) : null;
...
-- alter (Postgres 9.6+; Neon runs 15) closes that gap on first connect.
